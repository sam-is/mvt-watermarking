...
namespace NetTopologySuite.IO.VectorTiles.Mapbox.Watermarking
{
    ...
    public static class MapboxTileWriterWM
    {
            ...
            ref int currentX, ref int currentY, int WatermarkInt, int DElementarySegmentsCount, int[] keySequence)
        {
            ...
            // сохраняем currentX и currentY, потому что tgt.Transform изменяет их по ссылкам
            ...
            // возвращаем currentX и currentY к начальным значениям
            ...
            X = position.x; Y = position.y; // теперь они на начальной точке лайнстринга
            ...
            if (keySequence[0] == WatermarkInt) 
            {
                ...
                encodedIndex += 5;
                sequenceIndexStart = 2; // первый реальный сегмент закодировали, цикл начнёт работу со второго
