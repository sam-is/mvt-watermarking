...
namespace NetTopologySuite.IO.VectorTiles.Mapbox.Watermarking
{
    public class MapboxTileReaderWM
    {
        ...
        /// <summary>
        /// »звлечение из фичи, если в ней лежит лайнстринг
        /// </summary>
        ...
        /// <summary>
        /// »звлекает ÷¬« из одной фичи с лайнстрингом
        /// </summary>
        /// <param name="tgs"></param>
        ...
        // “ут будет извлечение, скорее всего
            ...
            ref int currentIndex, ref int currentX, ref int currentY, NoDistortionWatermarkOptions options, int[] keySequence)
        {
            ...
            {
                Console.WriteLine("ѕарсим MoveTo-LineTo"); // отладка

                // ≈сли в первый сегмент встроено, то команда == LineTo
                ...
                if (firstSegmentEncoded)
                {
                    ...
                    Console.WriteLine("ѕервый закодированный сегмент рассмотрен"); // отладка
                ...
                else if (command == MapboxCommandType.MoveTo)
                {
                    ...
                    Console.WriteLine("ќчередной закодированный сегмент рассмотрен"); // отладка
                }
                else
                {
                    Debug.Assert(command == MapboxCommandType.LineTo, "ну и че");
                    Console.WriteLine($"ѕервый сегмент не закодирован, втора€ команда - LineTo, count = {count}"); // отладка
                }
            ...
            var realSegmentsInONEElemSegment = options.D / realSegmentsNum;

            // предусмотреть возможность отражени€ лайнстринга
